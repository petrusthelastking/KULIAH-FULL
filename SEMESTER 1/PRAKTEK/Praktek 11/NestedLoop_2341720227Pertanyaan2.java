...
public class NestedLoop_2341720227Pertanyaan1 {
    ...
    public static void main(String[] args) {
        ...
        // Menampilkan array dengan foreach dan rata-rata
        for (int i = 0; i < temps.length; i++) {
            System.out.print("Kota ke-" + (i + 1) + ": ");
            double totalSuhu = 0.0;
            for (double suhu : temps[i]) {
                System.out.printf("%.2f ", suhu); // Format dua desimal
                totalSuhu += suhu;
            }
            double rataRata = totalSuhu / temps[i].length;
            System.out.printf("Rata-rata: %.2f%n", rataRata); // Format dua desimal
        }
    }
}
